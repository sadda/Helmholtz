function [phi, t] = ProjectedGradients3(TriInfo, Transformation, matrices, material, constants, dirName, IterMax, drawResults, phi, tInitial)
...
%         size(matrices.Mloc)
%         size(gradient)
%         rieszGradient = matrices.Mloc \ gradient;
...
rieszGradient = ComputeRieszGradient(gradient, TriInfo, matrices);
    ...
    [J, ~, J1, J2, J3] = ComputeData2(phi,TriInfo,Transformation,matrices,constants,material,0,dataEigen);
    ...
    if drawResults
        ...
        for iteration=iterationAll
            ...
            [~, ~, ~, ~, ~, ~, ~, ~, u, Theta] = ComputeData2(phi,TriInfo,Transformation,matrices,constants,material,0,dataEigen);
...
function phiNorm = ComputePhiNormSquare(phi, TriInfo, matrices)
    ...
    phiNorm = phiProlonged'*matrices.Mloc*phiProlonged / 2;
end
